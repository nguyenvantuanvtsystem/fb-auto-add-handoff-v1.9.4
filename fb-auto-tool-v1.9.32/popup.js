...
const trendTargetGroups=$("trendTargetGroups"),trendInterDelay=$("trendInterDelay");
...
let trendGroups=[],trendLearnRunning=false,trendPostRunning=false;
...
function renderTrendLearned(posts){
  ...
  if(!rows.length){box.innerHTML='<div class="hint" style="padding:6px">Chưa học được bài nào</div>';return;}
  box.innerHTML=rows.slice(-50).reverse().map((p,i)=>{
    ...
    return `<div style="padding:5px;border-bottom:1px solid #eee;font-size:12px"><b>${rows.length-i}. [${src}]</b> ${text}…${a}</div>`;
  }).join("");
}
...
[trendSourceLinks,trendTargetLinks,trendPerGroup,trendTargetGroups,trendInterDelay,trendStyleProfile,trendAnonymousEnabled].forEach(el=>el&&el.addEventListener("change",persistTrendDraft));
...
async function trendBuildRewrite(showStatus=true){
  ...
  const posts=Array.isArray(saved.trendLearnPosts)?saved.trendLearnPosts:[];
...
if(trendPostBtn)trendPostBtn.onclick=async()=>{
  ...
  try{
    ...
    const saved=await chrome.storage.local.get(["trendLearnPosts"]);
    const posts=Array.isArray(saved.trendLearnPosts)?saved.trendLearnPosts:[];
    if(!posts.length)throw new Error("Chưa có bài đã học. Hãy học bài trước.");
    const source=trendSourceText(posts);
    const prompt=trendRewritePrompt();
    const background=readTrendBackgroundConfig();
    const aiConfig=await getUnifiedAiConfig();
    ...
    trendLearnStatus.textContent=`AI đang viết ${groups.length} biến thể từ bài đã học...`;
    const runtimeAi={provider:aiConfig.provider,model:aiConfig.model,url:aiConfig.url};
    const drafts=[];
    for(let i=0;i<groups.length;i++){
      ...
      if(groups[i].manualLink&&!groups[i].explicitName){
        drafts.push("");
        trendLearnStatus.textContent=`Sẽ lấy tên thật của nhóm ${i+1}/${groups.length} khi mở nhóm...`;
        continue;
      }
      const res=await chrome.runtime.sendMessage({action:"aiRewriteTrend",sourceText:source,groupName:groups[i].name,prompt,aiConfig:runtimeAi,styleProfileId:trendStyleProfile.value||"",variant:i+1,background});
      if(!res?.ok)throw new Error(`AI lỗi ở nhóm ${groups[i].name}: ${res?.error||"unknown"}`);
      drafts.push(res.content);
      trendLearnStatus.textContent=`AI đã viết ${i+1}/${groups.length} biến thể...`;
    }
    await chrome.storage.local.set({trendRewriteDrafts:drafts});
    if(trendPreview){const previewDraft=drafts.find(Boolean)||"AI sẽ lấy tên thật của nhóm sau khi mở từng link đích.";trendPreview.textContent=`Đã chuẩn bị ${drafts.length} biến thể. Bản đầu cho "${groups[0].name}":\n\n${previewDraft}`;trendPreview.classList.remove("hidden");}
    const tab=await getActiveTab();if(!tab)throw new Error("Không tìm thấy tab Facebook");
    const runId=`trend-post-${Date.now()}-${Math.random().toString(36).slice(2,8)}`;
    const cfg={groups,delay:0,drafts,sourceText:source,prompt,styleProfileId:trendStyleProfile.value||"",interDelay:Math.min(3600,Math.max(5,parseInt(trendInterDelay.value)||30)),anonymousMode:!!(trendAnonymousEnabled&&trendAnonymousEnabled.checked),background};
    await persistTrendDraft();
    await chrome.storage.local.set({trendPostConfig:{groups,drafts,sourceText:cfg.sourceText,prompt:cfg.prompt,styleProfileId:cfg.styleProfileId,interDelay:cfg.interDelay,anonymousMode:cfg.anonymousMode,background:cfg.background},trendPostRunId:runId,trendPostOwnerTabId:tab.id,trendPostActive:true,trendPostIndex:0,trendPostDone:0,trendPostSkipped:0,trendPostTotal:groups.length,trendPostNextAt:0,trendPostLastColor:"",trendPostStage:"",trendPostSubmitIndex:-1,trendPostSubmitDispatchedAt:0,trendPostAnonymousMode:cfg.anonymousMode?"anonymous":"normal",trendPostStatus:`Đang chuẩn bị đăng ${groups.length} nhóm...`});
    trendPostStatus.textContent=`Đang chuẩn bị đăng ${groups.length} nhóm...`;
    ...
    const postConfig={groups,drafts,sourceText:cfg.sourceText,prompt:cfg.prompt,styleProfileId:cfg.styleProfileId,interDelay:cfg.interDelay,anonymousMode:cfg.anonymousMode,background:cfg.background};
...
try{
  chrome.storage.sync.get(["trendPerGroup","trendPrompt","trendGroupPrompt","trendSourceLinks","trendTargetLinks","trendStyleProfile","trendTargetGroups","trendInterDelay","trendLearnSourceMode","trendLearnFromTarget","trendBackgroundEnabled","trendBackgroundMode","trendBackgroundFixedColor","trendBackgroundColors","trendBackgroundMaxChars"],r=>{
    ...
    if(r.trendInterDelay!==undefined&&trendInterDelay)trendInterDelay.value=r.trendInterDelay;
    ...
    if(trendAnonymousEnabled)trendAnonymousEnabled.checked=r.trendAnonymousMode===true;
